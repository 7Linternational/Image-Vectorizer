...
Vectorizer.prototype.go = function(isLocal, callback) {
    var _this = this;
    _this.loadPixels(isLocal, function(ok1) {
        if (!ok1) {
...
Vectorizer.prototype.loadLocalPixels = function(callback) {
    var _this = this;
    console.log("loading local pixels");
    var buf = Buffer.from(this.url, 'base64');
    console.log(buf);
    // do stuff
    var type = imageType(buf);
    if (type.ext === "png") {
        try {
            //var _Data = fs.readFileSync(currentImg);
            /*Jimp.read(buf).then(function(err, image) {
                console.log("image is: ", err, image);
            }).catch(function(err) {
                console.error('png crash', e, e.stack);
            });*/
            var t = apngCanvasParser(buf);
        ...
        try {
            //var jpegData = fs.readFileSync(currentImg);
            //var rawImageData = jpegasm.decode(this.url);
...
Vectorizer.prototype.loadPixels = function(isLocal, callback) {
    var _this = this;
    console.log("is local: ", isLocal, (isLocal === true), Boolean(isLocal));
    if (isLocal === "true") {
        _this.loadLocalPixels(callback);
    } else {
